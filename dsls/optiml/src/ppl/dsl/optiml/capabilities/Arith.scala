...
//  def infix_exp[T](lhs: Rep[T])(implicit a: Arith[T], mT: Manifest[T]) = a.exp(lhs)
  ...
  implicit def vectorArith[T:Arith:Manifest] : Arith[Vector[T]] = new Arith[Vector[T]] {
    ...
    //                                               else a+b
    def +=(a: Rep[Vector[T]], b: Rep[Vector[T]])(implicit ctx: SourceContext) = a += b 
    def +(a: Rep[Vector[T]], b: Rep[Vector[T]])(implicit ctx: SourceContext) = a+b
    def -(a: Rep[Vector[T]], b: Rep[Vector[T]])(implicit ctx: SourceContext) = a-b
    def *(a: Rep[Vector[T]], b: Rep[Vector[T]])(implicit ctx: SourceContext) = a*b
    def /(a: Rep[Vector[T]], b: Rep[Vector[T]])(implicit ctx: SourceContext) = a/b

    def abs(a: Rep[Vector[T]])(implicit ctx: SourceContext) = a.abs
    def exp(a: Rep[Vector[T]])(implicit ctx: SourceContext) = a.exp
    ...
    def zero(a: Rep[Vector[T]])(implicit ctx: SourceContext) = ZeroVector[T](a.length)
}
