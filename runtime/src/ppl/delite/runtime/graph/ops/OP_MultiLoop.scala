...
class OP_MultiLoop(val id: String, val size: String, val sizeIsConst: Boolean, func: String, private[graph] val outputTypesMap: Map[Targets.Value,Map[String,String]], val needsCombine: Boolean, val needsPostProcess: Boolean, val needsPostProcess2: Boolean) extends OP_Executable {
  ...
  def chunk(i: Int): OP_MultiLoop = {
    ...
    val r = new OP_MultiLoop(id+"_"+i, size, sizeIsConst, function, Targets.unitTypes(id+"_"+i, outputTypesMap), needsCombine, needsPostProcess, needsPostProcess2) //chunks all return Unit
