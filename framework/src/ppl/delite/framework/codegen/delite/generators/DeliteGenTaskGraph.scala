...
trait DeliteGenTaskGraph extends DeliteCodegen with LoopFusionOpt {
  ...
  override def emitFatNode(sym: List[Sym[Any]], rhs: FatDef): Unit =
    emitFatNodeAny(sym, rhs)
   
  private def emitFatNodeAny(sym: List[Sym[Any]], rhs: AnyRef): Unit = {
    ...
    rhs match {
      case Reflect(s, u, effects) =>
        ...
        super.emitFatNode(sym, rhs.asInstanceOf[FatDef]); return
...
//          case op:DeliteFatOp => true
        ...
        gen.emitFatNode(sym, rhs.asInstanceOf[FatDef])
        bodyStream.flush
        ...
        // emit kernel
        gen.emitKernelHeader(sym, inVals, inVars, resultType, resultIsVar, external)
        kstream.println(bodyString.toString)
        gen.emitKernelFooter(sym, inVals, inVars, resultType, resultIsVar, external)
        
        if (hasOutputSlotTypes)
          gen.emitFatNodeKernelExtra(sym, rhs.asInstanceOf[FatDef]) // activation record class declaration
