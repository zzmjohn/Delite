...
//          stream.println(/*{*/"}")
  ...
  private def emitAnyKernelExtra(symList: List[Sym[Any]], rhs: AnyRef) : Unit = rhs match{
    case op: AbstractFatLoop => 
    ...
    case op: Def[_] => 
        super.emitNodeKernelExtra(symList, op)
    case op : FatDef => 
        super.emitFatNodeKernelExtra(symList, op)

  }

  override def emitFatNodeKernelExtra(symList: List[Sym[Any]], rhs: FatDef): Unit = emitAnyKernelExtra(symList, rhs)
  /*rhs match {
    case op: AbstractFatLoop =>
      stream.println("//activation record for fat loop")
      emitAbstractFatLoopKernelExtra(op, symList)
  ...
  */
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case zip: DeliteOpZipWith[_,_,_,_] => {
      ...
      stream.println(addTab()+"if( %s < %s_dcSize(%s) ) {".format(currDimStr,remap(zip.inA.tp),quote(zip.inA)))
      tabWidth += 1
