...
object DeliteBuild extends Build {
  ...
  */
  ...
  lazy val dsls = Project("dsls", file("dsls"), settings = virtBuildSettings) aggregate(optiml, optiql) 
  lazy val optiml = Project("optiml", file("dsls/optiml"), settings = virtBuildSettings) dependsOn(framework, deliteTest)
  lazy val optiql = Project("optiql", file("dsls/optiql"), settings = virtBuildSettings) dependsOn(framework, deliteTest)

  lazy val apps = Project("apps", file("apps"), settings = virtBuildSettings) dependsOn(optiml, optiql)
